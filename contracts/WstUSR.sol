...
/**
 * @title Wrapped stUSR Contract (wstUSR)
 * @dev
 * - WARNING: The wstUSR contract depends on the stUSR share price being stable and sufficiently liquid.
 * - Before deploying this contract, the `StUSR` contract should be seeded with an adequate initial liquidity deposit
 *   to establish a stable baseline for the `StUSR` share price and prevent potential manipulation (inflation attack).
 */
contract WstUSR is IWstUSR, ERC20PermitUpgradeable, AccessControlDefaultAdminRulesUpgradeable, IDefaultErrors {
    ...
    function initializeV2(address _admin, address _blacklister) external reinitializer(2) {
        _assertNonZero(_admin);
        _assertNonZero(_blacklister);
    ...
    function burnFromBlacklist(address _account) external onlyRole(BLACKLISTER_ROLE) {
        _assertNonZero(_account);
        if (!blacklisted[_account]) revert NotBlacklisted(_account);
        uint256 wstUSRAmount = balanceOf(_account);
        _assertNonZero(wstUSRAmount);

        uint256 stUSRSharesAmount = wstUSRAmount * ST_USR_SHARES_OFFSET;
        uint256 usrAmount = IERC20Rebasing(stUSRAddress).convertToUnderlyingToken(stUSRSharesAmount);

        super._update(_account, address(0), wstUSRAmount);
        IStUSR(stUSRAddress).withdraw(usrAmount, address(this));
        ISimpleToken(usrAddress).burn(address(this), usrAmount);

        emit BurnedFromBlacklist(_account, wstUSRAmount);
    }
