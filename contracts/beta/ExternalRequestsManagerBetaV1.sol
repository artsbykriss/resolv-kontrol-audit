...
import {IExternalRequestsManagerBetaV1} from "./interfaces/IExternalRequestsManagerBetaV1.sol";
import {IAddressesWhitelist} from "../interfaces/IAddressesWhitelist.sol";

contract ExternalRequestsManagerBetaV1 is IExternalRequestsManagerBetaV1, AccessControlDefaultAdminRules, Pausable {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _allowedTokenAddresses.length; i++) {
            _assertNonZero(_allowedTokenAddresses[i]);
            allowedTokens[_allowedTokenAddresses[i]] = true;
        }

        isWhitelistEnabled = true;
    }

    function setTreasury(address payable _treasuryAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function removeAllowedToken(address _allowedTokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit AllowedTokenRemoved(address(_allowedTokenAddress));
    }
    ...
    function requestMint(
        ...
        Request memory request = _addMintRequest(_depositTokenAddress, _amount);

        emit MintRequestCreated(request.id, request.provider, request.token, request.amount);
    }
    ...
    function cancelMint(uint256 _id) external onlyAllowedProviders mintRequestExist(_id) {
        ...
        depositedToken.safeIncreaseAllowance(address(this), request.amount);
    ...
    function completeMint(
        ...
        depositToken.safeIncreaseAllowance(address(this), request.amount);
    ...
    function requestBurn(
        ...
        Request memory request = _addBurnRequest(_withdrawalTokenAddress, _issueTokenAmount);

        emit BurnRequestCreated(request.id, request.provider, request.amount, request.token);
    }
    ...
    function cancelBurn(uint256 _id) external onlyAllowedProviders burnRequestExist(_id) {
        ...
        request.state = State.CANCELLED;
        IERC20 issueToken = IERC20(ISSUE_TOKEN_ADDRESS);
        issueToken.safeIncreaseAllowance(address(this), request.amount);
    ...
    function completeBurn(
        ...
        ISimpleToken issueToken = ISimpleToken(ISSUE_TOKEN_ADDRESS);
        issueToken.burn(address(this), request.amount);

        IERC20(request.token).safeTransferFrom(msg.sender, request.provider, _withdrawalAmount);
    ...
    function emergencyWithdraw(IERC20 _token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        _token.safeIncreaseAllowance(address(this), balance);
