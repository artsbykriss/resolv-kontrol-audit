...
import {IResolvStakingV2} from "../interfaces/staking/IResolvStakingV2.sol";

/**
 * @title ResolvStakingV2
 * @notice V2 staking contract for RESOLV tokens with reward distribution and optional transferability.
 * @dev This version introduces the ability to enable or disable token transferability by admin control.
 */
/// @custom:oz-upgrades-unsafe-allow external-library-linking
contract ResolvStakingV2 is IResolvStakingV2, ERC20PermitUpgradeable, ERC20VotesUpgradeable, AccessControlDefaultAdminRulesUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     * @notice Enables or disables token transferability.
     * @param _enabled If true, enables transfers; if false, disables transfers.
     */
    function setTransferEnabled(bool _enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
        transferEnabled = _enabled;
        emit ResolvStakingEvents.TransferEnabledSet(_enabled);
    }
    ...
    /**
    * @dev Allows token transfers only when `transferEnabled` is true, otherwise reverts with a NonTransferable error.
    */
    function transfer(address _to, uint256 _value) public nonReentrant override returns (bool) {
        require(transferEnabled, ResolvStakingErrors.NonTransferable());
    ...
    * @dev Allows token transfers only when `transferEnabled` is true, otherwise reverts with a NonTransferable error.
    */
    function transferFrom(address _from, address _to, uint256 _value) public nonReentrant override returns (bool) {
        require(transferEnabled, ResolvStakingErrors.NonTransferable());
